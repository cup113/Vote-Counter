...
function fresh() {
	...
	if ($votetips[0].children.length !== 0) {
		var $firstspan: HTMLElement = ($votetips[0].children.item(0) as HTMLElement),
		opacity = parseFloat($firstspan.style.opacity),
		opacity_reduce = $votetips[0].children.length * 0.01;
		if (opacity > opacity_reduce) $firstspan.style.opacity = (opacity - opacity_reduce).toString();
		else $votetips[0].removeChild($firstspan);
	}
	if (Ele.rankAnimationPlaying) {
		...
		for (let e of Ele.electors) {
			...
			ey = eoffset.top,
			el = e.get_location(),
			ty = el % cellsPerCol * cellHeight + ry,
			tx = Math.floor(el / cellsPerCol) * cellWidth + rx,
			espeed = e.speed;
			if (Math.abs(ex - tx) <= 0.5) { espeed.x = 0; }
			else {
				let diffx = tx - ex;
				espeed.x = (diffx > 0)? Math.min(espeed.x + 4, Math.ceil(diffx / 5)): Math.max(espeed.x - 4, Math.floor(diffx / 5));
			}
			if (Math.abs(ey - ty) <= 0.5) { espeed.y = 0; }
			else {
				let diffy = ty - ey;
